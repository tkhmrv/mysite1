<?php
function sfsi_plus_forum_notification_popup() {
	...
	$sfsi_plus_installDate_new = new DateTime( $sfsi_plus_installDate );

	$sfsi_plus_installDate_only = $sfsi_plus_installDate_new->format( 'Y-m-d' );

	$sfsi_plus_installDate_string = strtotime( $sfsi_plus_installDate_only );
	$sfsi_plus_current_Date_string = strtotime( date( 'Y-m-d' ) );

	if ( $sfsi_plus_current_Date_string == $sfsi_plus_installDate_string ) {
		$option_name = 'sfsi_plus_show_admin_popup';
		$new_value = 'yes';
	...
	/* Check WP language */
	$sfsi_plus_language = get_bloginfo( 'language' );
	$sfsi_plus_allowed_languages = array( 'en-US', 'en-AU', 'en-GB', 'en-NZ', 'en-CA', 'en-ZA' );

	if ( ! in_array( $sfsi_plus_language, $sfsi_plus_allowed_languages ) ) {
		return;
	}

	$sfsi_plus_show_admin_popup = get_option( 'sfsi_plus_show_admin_popup' );
	if( 'yes' !== $sfsi_plus_show_admin_popup ) {
		return;
	}

	$option_5 = maybe_unserialize(get_option('sfsi_plus_section5_options',false));
	$sfsi_plus_show_admin_popup_option_5 = isset($option_5['sfsi_plus_show_admin_popup']) ? $option_5['sfsi_plus_show_admin_popup'] : 'yes';
	if( 'yes' !== $sfsi_plus_show_admin_popup_option_5) {
		return;
	}
	...
	$sfsi_plus_default_hide_admin_notification_class = '';
	$sfsi_plus_default_hide_admin_notification = get_option( 'sfsi_plus_default_hide_admin_notification' );
	if( 'hide' === $sfsi_plus_default_hide_admin_notification ) {
		$sfsi_plus_default_hide_admin_notification_class = ' usm-plus-widget--open';
	}
?>
	<div id="usm-plus-admin-notification-widget" class="usm-plus-widget<?php echo esc_attr( $sfsi_plus_default_hide_admin_notification_class ); ?>">
		<div class="usm-plus-widget__body">
			<button type="button" class="usm-plus-widget__toggle-btn">
				<svg id="icon-btn-arrows" class="usm-plus-widget__toggle-btn-icon toggle-btn-icon" viewBox="0 0 19 11" width="19" height="11">
			...
			<div class="usm-plus-widget__content">
				<div class="usm-plus-widget__text-content">
					<p class="usm-plus-widget__title"><?php _e( 'Do the social media icons show like you want to?', 'ultimate-social-media-plus' ); ?></p>

					<p class="usm-plus-widget__text"><?php
						printf(
							__( 'If not, ask us in the %1$sforum%2$s, we\'re happy to help – %3$squickly & for free!%4$s','ultimate-social-media-plus' ),
							'<a class="usm-plus-widget__text-link" href="https://goo.gl/auxJ9C#no-topic-0" target="_blank">',
							'</a>',
							'<b>',
							'</b>'
						);
					?></p>
					<p class="usm-plus-widget__text"><?php _e( 'We can also consult you how to place them for maximum effect & assist with anything else.', 'ultimate-social-media-plus' ); ?></p>
					<a href="https://goo.gl/auxJ9C#no-topic-0" class="usm-plus-widget__main-btn" target="_blank">
					<?php _e( 'Ask in forum', 'ultimate-social-media-plus' ); ?>
					<svg class="usm-plus-widget__main-btn-icon" id="icon-arrow-right" viewBox="0 0 6 10" width="6" height="10">
					...
					<div class="usm-plus-widget__info usm-plus-widget-info">
						<div class="usm-plus-widget-info__title">
							<?php _e( 'Trouble logging in?', 'ultimate-social-media-plus' ); ?>
							<svg class="usm-plus-widget-info__icon" id="icon-info" viewBox="0 0 12 12" width="12" height="12">
						...
						<p class="usm-plus-widget-info__popup"><?php _e( 'Your account on Wordpress.org (where you open a new support thread) is different to the one you login to your WordPress dashboard (where you are now). If you don\'t have a WordPress.org account yet, please sign up at the top right on the Support Forum page, and then scroll down on that page . It only takes a minute :)', 'ultimate-social-media-plus' ); ?>
							<br>
							<br><?php _e( 'Thank you!', 'ultimate-social-media-plus' ); ?>
						</p>
					</div>
				</div>
				<footer class="usm-plus-widget__footer">
					<p class="usm-plus-widget__footer-text"><?php
						printf(
							__( 'This widget is %1$sonly visible to you as admin%2$s. You can %3$shide it forever.%4$s','ultimate-social-media-plus' ),
							'<b>',
							'</b>',
							'<a class="usm-plus-widget__footer-text-link usm-plus-widget__text-link" href="javascript:void(0)">',
							'</a>'
						);
					?></p>
				</footer>
				<p class="usm-plus-widget__footer-bottom-text"><?php _e( 'By Ultimatelysocial', 'ultimate-social-media-plus' ); ?></p>
			</div>
		</div>
	</div>
<?php
...
add_action( 'wp_ajax_sfsi_plus_default_hide_admin_notification', 'sfsi_plus_default_hide_admin_notification_callback' );

function sfsi_plus_default_hide_admin_notification_callback() {
	...
	$option_name = 'sfsi_plus_default_hide_admin_notification' ;
